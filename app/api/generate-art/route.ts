...
export async function POST(req: NextRequest) {
  try {
    ...
    // ←— هنا زدنا الفانكسيون القديمة لي كانت عندك (تحول output ل dataURL باش frontend يقدر يعرض الصورة)
    // extract URL (supports array or object shapes)
    ...
    // if still not string, try to find URL in JSON
    ...
    if (!imageUrl) {
      // no URL found — return raw replicate output for debugging
    ...
    try {
      ...
      if (!imageResponse.ok) {
        return NextResponse.json(
          { error: `Failed to fetch generated image: ${imageResponse.status}`, replicate_output: output },
          { status: 502 }
        );
      }
      ...
      return NextResponse.json({
        ...
        success: true,
        debug: { faceX: merged.faceX, faceY: merged.faceY, faceSize: merged.faceSize },
      });
    } catch (e: any) {
      console.error("Failed to fetch/convert generated image:", e);
      return NextResponse.json({ error: "Failed to fetch generated image", replicate_output: output }, { status: 502 });
    }
